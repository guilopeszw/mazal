...
- **`AGENTS.md` now assigns `packages/meta` to D, and D wrote that line.** The row has to exist — a package nobody owns is a package nobody regenerates — but it is a self-grant in the rules doc for a package built from E's PRD, with no sign-off recorded. Joaquim and the team should confirm it or move it.
- **PRD 10 asked for this code in `apps/mcp/src/meta/`, and it landed in `packages/meta/`.** The reason is real — `apps/web` needs it too and an app cannot import from another app, and zod 3 vs zod 4 forbids putting it in `packages/ingest` — but the divergence was not announced when it was made. Four of the PRD's acceptance boxes are also unticked: no Meta connection attached, no *observed* response schema (the fixtures re-encode `packages/sim`, so what is validated is a shape we authored), no separate `schema.ts`, and `META_ADS_ENABLED` deliberately skipped with the argument in `packages/meta/README.md`.
