...
## 2026-08-10 22:20 BRT · Guilherme's agent · five merged, and the pre-flight stops asking for a file

**Done:** #60, #61, #62, #63 and #64 are **merged**, `stage` is at 322 tests across 43 files with typecheck, `next build` and both fixture guards green, and **production is redeployed** — page 200, `POST /api/chat` 200, and the plan action now reads *"Priorize mostrar o prazo de entrega na página do produto."* rather than an English title inside Portuguese prose.

**The product gap is closed.** #66 is merged and deployed; `stage` is at `91c3ea6`. The pre-flight was reachable only after a CSV had parsed, which put the one answer an off-pixel seller can get behind the one artefact they cannot produce: iFood and WhatsApp put no pixel on the checkout, Meta reports zero for every conversion column, `diagnose` refuses that export by design, and the seller had to hand over a file Mazal would tell them it could not read to reach the question that never needed the file. `preflightUpload` always took the card and no days — only the layout was in the way. The product form now always renders, the drop zone becomes the optional half, and *"Diagnose this campaign"* stays gated on real rows and says why.

**Three defects this session were checks that could not fail, and two of them were mine.** `drive.mjs` — the tool written to stop exactly this — exited 0 against a dead server (Chrome fires `Page.loadEventFired` for its own error page, so the script reported that page's text as the measurement), exited 0 on any CDP protocol error, and on a hardcoded port 9222 attached to a *foreign* Chrome, navigating a browser it had not launched. All three fixed in #60 and each watched to fail first.

The third was my own control for #66. I ran the no-CSV probe against the pre-change build in production and read `formVisibleWithNoCsv: false` as evidence — but this PR also changed the paperclip's `aria-label`, so on the old build the probe never opened the panel at all. Both flags were false for a reason with nothing to do with the form. **A control that cannot distinguish "the thing is missing" from "I never looked" is not a control.** It now runs against a local build of the parent commit beside the branch, asserts `panelOpened`, and measures a box with non-zero width and height rather than mere presence. That assertion paid immediately: the rewritten probe's regex missed the parent's *"Meta Ads CSV export"* label and returned `openerFound: false`, which without the flag I would have read as the form being absent.

**A fourth was found by a reviewer and rated below their own reporting bar; it was worth the three lines.** `setCategory` is synchronous, so both buttons went live while the three category-derived fields were still empty — and the fix above had just made them `required`. One frame after the change event: `preflightEnabled: true, emptyRequiredBoxes: 3, formValid: false`. That is the first thing an off-pixel seller does in this panel, and the failure is the browser refusing the click over three boxes they were never asked to fill. Closed with the `busy` state already in the component.

**Verified on production, not on localhost.** `page 200`, `POST /api/chat 200`, and with no file handed over at any point: form present and interactive, drop zone still there, pre-flight enabled after a category and three fields, diagnosis still disabled, answer rendering — *"Launch small. Break-even is 3.33× and the likely case is 1.11×."*

**Next:** one pass to put `apps/web` into Portuguese — see below. It is the largest remaining gap between what Mazal is and what a Brazilian seller sees.

**Blocked / watch out:** five things.

- **The web UI is in English and only `/api/chat` speaks Portuguese.** The landing reads *"Campaigns shouldn't need luck"*, the chips are English, and a chip answer comes back *"Your ads are fine. Your checkout broke on 12 July."* The typed-question path answers in Portuguese; the pre-computed demo answers do not. For a product whose seller is Brazilian this is the largest remaining honesty gap on screen, and #66 adds more English copy because it follows the file it sits in. It wants one pass over `apps/web`, not a branch about something else.
- **The plan loader's `log` variant lost its inversion in *both* themes, and light never had the bug.** The 21:30 entry called it a deliberate loss and it is, but the loss is wider than that entry says: in light mode the face ramped to `--pl-inv-face`, which is `#1d211c` there — a near-black panel on cream, which was correct and is now gone too. (An earlier draft of this line said `rgb(33,37,32)`. That figure came from someone else's rendered-pixel measurement, four per channel off the token, and I could not reproduce it; the token is what the repo can be checked against, so the token is what this says.) Scoping the drop to dark would have kept it. `.pl-face[data-invert="true"]` and 21 `--pl-inv-*` declarations are now unreachable; the rule says so on its face rather than being deleted, because they are the palette a restored inversion would use.
- **`form.reportValidity()` does not constrain an empty non-required number input.** `min` only bites on a value that is there, so a cleared assumed field passed validation, arrived as `Number("") === 0`, and came back as *"Gross margin must be greater than 0"* — a zod message for a field nobody said was mandatory, with the badge reading YOURS over an empty box. Fixed with `required` on all seven. The general shape is worth remembering: **native validation is not a guard against absence unless you ask for one.**
- **A guard and the control it guards drifted apart.** The upload's submit handler tested `!parsed` while the button beside it disabled on `!hasRows`, so a file that parsed to zero rows passed one and not the other. Unreachable today — Chrome declines implicit submission while the default button is disabled — but that is the browser holding the invariant, not the code.
- **Once a file has parsed, the drop zone disappears and there is no way to replace it without closing and reopening the panel.** Pre-existing, not a regression, and left alone deliberately — but this session made the panel somewhere a seller lingers rather than passes through, so it is likelier to be met now than it was.
...
Found by asking what a real user would get. The user is a friend's bakery selling frozen goods through **iFood and WhatsApp**: no pixel on either checkout, so Meta reports spend, impressions, reach and link clicks honestly and reports zero for every conversion column. Fed that export, `diagnose` returned **stage 3, `thin_pdp`** — *your product page is too thin, that is why nobody adds to cart*. Meta had never seen a cart. Confident, wrong, and actionable: the seller rewrites a page that was never the problem.

The minimum-sample gate cannot catch it. Stage 3 is gated on **clicks**, which are plentiful; the *numerator* is what is absent. And this is not an edge case here — iFood, WhatsApp, Instagram DMs and marketplaces are how a large share of Brazilian small sellers transact.
...
**And that file is now provably in sync with Studio, not asserted to be.** Pull `metadata.instructions` with `COLLECTION_VIRTUAL_MCP_GET` on `vir_s9bfvfwe5vloXTD6Ttn_o`, extract the four-backtick block out of the doc, `diff` the two: identical. Run it whenever that file is touched — Studio keeps no history, so this is the only check it has. If you want a size to compare, say which one you mean: the block is **6246 bytes** (`wc -c`, minus the trailing newline) and **6197 characters** to JavaScript, because `—`, `–` and `ã` are one character and several bytes each. The whole 49-byte gap is those: fourteen em dashes and three en dashes at three bytes, fifteen accented letters at two. (`R$` is not one of them — it is two plain ASCII bytes, and I had it in this list until a review took the list seriously.) A count with no unit on it is how the next person concludes the file drifted when it did not.

**A correction to the 22:10 entry below, and a correction to my own first draft of this one.** That entry says *"60 tests in `@mazal/mcp` (was 40), 195 at the root"*, and I first wrote here that none of it reproduced anywhere. **That was wrong, and it was the same mistake I was accusing it of.** At `15caa68` — the commit that entry was written on — those commands print exactly **60** and **195**. The numbers moved to **68** and **294** when `53bbff3` merged `main` in. The one figure that does not reproduce is *"was 40"*: the branch's base `a967c6b` prints **45**.

So the lesson is not that the entry lied. It is that **a test count with no commit beside it stops being true without anyone editing it** — and then it reads as a fabrication to whoever checks next. Quote the commit, or quote the command and the day.

Separately: the `optimization.md` §4 citation is in [`deco-agent.md`](deco-agent.md), not in the entry below — I misattributed it. The file is real, tracked at the repo root, and referenced from `docs/allocator.md`, `docs/allocator-results.md`, `docs/deck.md` and `docs/deco-agent.md`. Nothing to fix there.
...
*The composer.* It sat in the flow and floated after the transcript; it now docks to the bottom of the viewport the moment the first turn lands (`371995f`), and slides there rather than jumping (`01c839f`). `position` is not interpolable, so the slide is a hand-written FLIP: `ask` records the bar's position in the click handler, a `useLayoutEffect` reads where it landed before the browser paints, and the gap plays back as a `translateY` that unwinds to zero over 380ms. **It measures only `.top`** — a horizontal shift cannot corrupt it, which is what let the sidebar push the page without touching it.

*The sidebar.* The header is now the mark alone; the wordmark and the `beta` badge live in a left panel the mark opens (`c8834ee` lifts `Mark` into `components/mark.tsx`, `fb034d2` adds `components/sidebar.tsx`). The panel's own mark stands on the same pixel as the header's — one `MarkButton` rendered in two mutually exclusive rows, both `px-5 py-3.5` with an `h-8` inner row, so the mark does not move when the panel arrives and only the wordmark unfolds. The body is empty on purpose: one ephemeral conversation and no account behind it, so there is no history to list. The theme toggle moved out of the header into a `Demo account` row at the panel's foot — that label is the honest one, there is no login, session or user record in this build.

*The integration.* `fix/ui-elements` → `stage` (`f275285`) → `main`, with `origin/main`'s E-agent sync (PR #19) merged in on the way (`eed6f1e`, `3f7ee35`). All three branches are pushed and identical in content; `fix/ui-elements` was fast-forwarded to `main` and holds nothing unique. `main` is green: **161/161 root tests, `apps/web` typecheck clean, `next build` exits 0.**

Open and unmerged: **`fix/sidebar-narrow-overlay`** (off `stage`, one commit `7357ace`) — pushed, PR not opened.

**Next:** review `fix/sidebar-narrow-overlay` and merge it to `stage`; the branch owner does not merge. It gates the sidebar's rail at `md`: above it the panel still pushes the page, below it the rail is nil and the panel overlays with a scrim. Without it the panel pushes at every width and a phone gets a horizontal scrollbar while it is open — 17rem is more than half a 375px screen, and under 428px the composer's two 36px buttons and their padding need 156px the remaining column no longer has. The breakpoint is the one judgement call in it and is a `md:` prefix in two places.

**Blocked / watch out:**

- **A deleted colour token fails silently, and it already caught me once.** The one-accent pass removed `--color-ref{,-soft,-ink}`; the sidebar's `beta` badge was still painted with `bg-ref-soft`/`text-ref-ink`, carried over from the header before the wordmark moved. Tailwind simply does not generate a class for a token that no longer exists — no background, no colour, **no error, and typecheck and build both stay green**. Fixed in the merge commit `0c040c0`. If you move any markup that predates that pass, grep it for `ref-` first.
