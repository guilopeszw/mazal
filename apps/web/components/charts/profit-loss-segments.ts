/**
 * Vendored from the bklit registry (`@bklit/profit-loss-line`, `src/charts/
 * profit-loss-segments.ts`). The shadcn CLI ships `profit-loss-line.tsx` without
 * this file, so the component it imports does not compile as installed; this is
 * the upstream source, unmodified, so a later `shadcn add --overwrite` replaces
 * it cleanly.
 */
