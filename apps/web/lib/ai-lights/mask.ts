// Reference implementation supplied by the product owner. Adapt, do not re-derive.
...
// The conic painting the stroke is a gradient of OPACITY, not colour. Opaque over
// roughly a third of the circle and gone across the rest — that partial coverage is
// what makes it read as a highlight travelling an edge rather than a lit border.
...
// A stroke of width 20 blurred by 32 is a soft BAND hugging the shape — that reads as
// light. The first entry has no stroke width: it fills the shape and punches a 1px hole
// inside it, and that carved band is the coloured border itself. Carve from a fill
// rather than stroking at width 1, or the line straddles the edge.
...
// The body returns to the same handful of shapes forever, so after one rotation every
// rebuild is a lookup.
...
export function buildMask(o: MaskOptions): string {
  ...
  // Each layer sits outside the element by its own padding, or the blurred stroke is
  // clipped square.
